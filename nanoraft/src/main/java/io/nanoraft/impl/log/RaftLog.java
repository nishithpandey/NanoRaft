...
import io.nanoraft.impl.util.ArrayRingbuffer;
 ...
 * followers in the same log order and all Raft nodes in a Raft group eventually
 * keep the exact same copy of the Raft log.
 * <p>
 * Raft maintains the following properties, which together constitute the
 * LogMatching Property:
 * <ul>
 * <li>If two entries in different logs have the same index and term, then they
 * store the same command.</li>
 * <li>If two entries in different logs have the same index and term, then the
 * logs are identical in all preceding entries.</li>
 * </ul>
 *
 * @see LogEntry
 * @see SnapshotEntry
...
public final class RaftLog {
    ...
    /**
     * Returns the log entry stored at {@code entryIndex}. Entry is retrieved only
     * from the current log, not from the snapshot entry.
     * <p>
    ...
    public LogEntry getLogEntry(long entryIndex) {
        if (entryIndex < FIRST_VALID_LOG_INDEX) {
            throw new IllegalArgumentException(
                    "Illegal index: " + entryIndex + ". Index starts from " + FIRST_VALID_LOG_INDEX);
    ...
    /**
     * Returns true if the log contains an entry at {@code entryIndex}, false
     * otherwise.
     * <p>
     * Important: Log entry indices start from 1, not 0.
     */
     ...
     *             If no entries are available to truncate, if {@code entryIndex} is
    ...
    public List<LogEntry> truncateEntriesFrom(long entryIndex) {
        if (entryIndex <= snapshotIndex()) {
            ...
            throw new IllegalArgumentException(
                    "Illegal index: " + entryIndex + ", last log index: " + lastLogOrSnapshotIndex());
        }
    ...
    /**
     * Returns the last entry index in the Raft log, either from the last log entry
    ...
    /**
     * Returns the last entry in the Raft log, either from the last log entry or
     * from the last snapshot if no logs are available.
     */
     ...
     *
     * @throws IllegalArgumentException
     *             If an entry is appended with a lower term than the last term in
     *             the log or if an entry is appended with an index not equal to
    ...
    /**
     * Returns the last term in the Raft log, either from the last log entry or from
     * the last snapshot
     */
    ...
    /**
     * Returns log entries between {@code fromEntryIndex} and {@code toEntryIndex},
     * both inclusive.
     *
     * @throws IllegalArgumentException
    ...
    public static int getLogCapacity(int commitCountToTakeSnapshot, int maxUncommittedLogEntryCount) {
        int maxLogEntryCountToKeepAfterSnapshot = Math.max(1,
                (int) (commitCountToTakeSnapshot * KEPT_LOG_ENTRY_RATIO_BEFORE_SNAPSHOT_INDEX));
