...
public enum RaftNodeStatus {
    ...
    /**
     * A Raft node stays in this status when there is no ongoing membership change
     * or a Raft group termination process. Operations are committed in this status.
     */
    ACTIVE,

    /**
     * A Raft node moves to this status when a Raft group membership change
     * operation is appended to its Raft log and stays in this status until the
     * membership change is either committed or reverted. New operations can be
     * replicated while there is an ongoing membership change in the Raft group, but
     * no other membership change, or leadership transfer can be triggered until the
     * ongoing membership change process is completed.
     */
