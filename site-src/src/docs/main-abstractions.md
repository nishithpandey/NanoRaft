...
NanoRaft's main abstractions are listed below.
...
Raft consensus algorithm and NanoRaft's implementation. Please check the
...
`RaftNode` differentiates members of a Raft group with a unique id. NanoRaft
...
<a
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/RaftRole.java"
target="_blank">`RaftRole`</a> denotes the roles of `RaftNode`s as specified in
the Raft consensus algorithm. Currently, NanoRaft implements the main roles
...
operations on them. Currently, NanoRaft supports memory-based state machines
...
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/model/log/BaseLogEntry.java"
...
href="https://github.com/NanoRaft/NanoRaft/tree/master/nanoraft/src/main/java/io/nanoraft/model"
target="_blank">`io.nanoraft.model`</a> for more details. In addition, there is
a <a
...
NanoRaft comes with a default POJO-style implementation of these interfaces
available under the <a
href="https://github.com/NanoRaft/NanoRaft/tree/master/nanoraft/src/main/java/io/nanoraft/model/impl"
target="_blank">`io.nanoraft.model.impl`</a> package. Users of NanoRaft can
...
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/model/RaftModelFactory.java"
target="_blank">`RaftModelFactory`</a>. NanoRaft requires a minimum set of
...
storage into a <a
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/persistence/RestoredRaftState.java"
...
<a
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/execution/RaftNodeExecutor.java"
...
NanoRaft contains a default implementation, <a
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/execution/impl/DefaultRaftNodeExecutor.java"
...
the use-cases. Users of NanoRaft can provide their own <a
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/execution/RaftNodeExecutor.java"
...
rules defined by NanoRaft.  

## `RaftException`

<a
href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/exception/RaftException.java"
target="_blank">`RaftException`</a> is the base class for Raft-related
exceptions. NanoRaft defines a number of <a
href="https://github.com/NanoRaft/NanoRaft/tree/master/nanoraft/src/main/java/io/nanoraft/exception"
...
In order to run a Raft group (i.e., Raft cluster) using NanoRaft, we need to:
...
* __(optional)__ implement the `RaftModel` and `RaftModelFactory` interfaces to
  ...
  `io.nanoraft.model.impl` package,
...
* __(optional)__ provide a `RaftStore` implementation if we want to restore
  ...
  about NanoRaft's fault tolerance capabilities.
...
* ![](/img/info.png){: style="height:25px;width:25px"} build a discovery and RPC
  ...
  because simplicity is the primary concern for NanoRaft's design philosophy.
  NanoRaft offers a minimum API set to cover the fundamental functionality and
  ...
  NanoRaft neither broadcasts the Raft group members to any external discovery
...
In the next section, we will build an atomic register on top of NanoRaft to
demonstrate how to implement and use NanoRaft's main abstractions.
...
![Architectural overview of a Raft group](/img/nanoraft_architectural_overview.png){: style="height:592px;width:800px"}
