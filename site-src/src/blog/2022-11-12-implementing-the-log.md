...
This article is the second in _the ins and outs of NanoRaft_ series. Here we
dissect the log.

NanoRaft implements the log with 2 components: [`RaftLog`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/impl/log/RaftLog.java) and [`RaftStore`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/persistence/RaftStore.java).
...
![Figure 1](https://nanoraft.io/img/blog3-fig1.png)
...
is specified by [`RaftConfig.getMaxPendingLogEntryCount()`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/RaftConfig.java#L213). Log entries reside in
this section until they are committed or truncated. When a client calls
[`RaftNode.replicate()`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/RaftNode.java#L258), if the _in progress_ section has no empty slots, the
client gets a [`CannotReplicateException`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/exception/CannotReplicateException.java). This exception means that the
...
client of the initial log entry gets a [`NotLeaderException`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/exception/NotLeaderException.java). This exception
...
moved to the _committed_ section. A new snapshot is taken from [`StateMachine`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/statemachine/StateMachine.java)
when the _committed_ section is full. The size of the _committed_ section is
specified by [`RaftConfig.getCommitCountToTakeSnapshot()`](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/RaftConfig.java#L231). Since all Raft nodes
...
a snapshot to those Raft nodes instead of the recent log entries. NanoRaft
...
[RaftStore](https://github.com/NanoRaft/NanoRaft/blob/v0.3/nanoraft/src/main/java/io/nanoraft/persistence/RaftStore.java) is designed to amortize the cost of disk writes. RaftStore has 2
...
NanoRaft amortizes the cost of disk writes by performing multiple
`RaftStore.persistLogEntry()` calls before a `RaftStore.flush()` call.

## Wrap up

In this article, we explored the details of NanoRaft's log implementation.
Next, we will investigate how NanoRaft realizes log replication.
