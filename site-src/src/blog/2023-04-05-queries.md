...
walk through the querying capabilities available in NanoRaft. You can see some
code samples <a href="https://nanoraft.io/docs/tutorial-building-an-atomic-register/#performing-queries" target="_blank">here</a>.

For impatient readers, TLDR is as follows:

- NanoRaft separates mutating and query operations via <a href="https://github.com/NanoRaft/NanoRaft/blob/031c049c2b8fc2cd2b356cbdee2091592c791651/nanoraft/src/main/java/io/nanoraft/RaftNode.java#L259" target="_blank">`RaftNode.replicate()`</a>
  and <a href="https://github.com/NanoRaft/NanoRaft/blob/031c049c2b8fc2cd2b356cbdee2091592c791651/nanoraft/src/main/java/io/nanoraft/RaftNode.java#L364" target="_blank">`RaftNode.query()`</a>
  APIs. These APIs return <a href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/Ordered.java" target="_blank">`Ordered`</a>
  objects. `Ordered` wraps the actual result of an operation along with
  the commit index at which the operation is executed. NanoRaft offers
  3 policies to execute queries with different consistency and performance
  characteristics via <a href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/QueryPolicy.java" target="_blank">`QueryPolicy`</a>:
...
- `QueryPolicy.LEADER_LEASE` enables the leader to locally execute queries
  ...
  on top of <a href="https://nanoraft.io/blog/2021-09-08-today-a-raft-follower-tomorrow-a-raft-leader/" target="_blank">the periodic heartbeating mechanism</a>.
  ...
  parameter, NanoRaft ensures that a new leader is not elected while
...
appended, NanoRaft can trigger the snapshotting mechanism to truncate the log
...
Here we explore how NanoRaft implements the techniques discussed in the Raft
dissertation. NanoRaft optimizes querying with 3 options, ranging from the one
...
guarantee which allows achieving monotonic reads. For this, NanoRaft separates
...
<a href="https://github.com/NanoRaft/NanoRaft/blob/031c049c2b8fc2cd2b356cbdee2091592c791651/nanoraft/src/main/java/io/nanoraft/model/message/AppendEntriesRequest.java" target="_blank">`AppendEntriesRequest`</a>
...
<a href="https://github.com/NanoRaft/NanoRaft/blob/031c049c2b8fc2cd2b356cbdee2091592c791651/nanoraft/src/main/java/io/nanoraft/model/message/AppendEntriesRequest.java#L55" target="_blank_">`AppendEntriesRequest.getQuerySequenceNumber()`</a>.
...
NanoRaft amortizes the cost of leadership confirmation for multiple queries.
...
it receives a majority ack. To prevent OOMs on the leader RaftNode, NanoRaft
limits the number of pending queries by
<a href="https://github.com/NanoRaft/NanoRaft/blob/031c049c2b8fc2cd2b356cbdee2091592c791651/nanoraft/src/main/java/io/nanoraft/RaftConfig.java#L115" target="_blank">`RaftConfig.getMaxPendingLogEntryCount()`</a>. 
If there is no available query slot, `RaftNode.query()` fails with
<a href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/exception/CannotReplicateException.java" target="_blank">`CannotReplicateException`</a>,
...
entry to discover the current commit index. In NanoRaft, this log entry
contains the operation provided by <a href="https://github.com/NanoRaft/NanoRaft/blob/031c049c2b8fc2cd2b356cbdee2091592c791651/nanoraft/src/main/java/io/nanoraft/statemachine/StateMachine.java#L149" target="_blank">`StateMachine.getNewTermOperation()`</a>.
...
leadership confirmation for inflight queries. In NanoRaft, a leader
...
all pending queries with <a href="https://github.com/NanoRaft/NanoRaft/blob/master/nanoraft/src/main/java/io/nanoraft/exception/NotLeaderException.java" target="_blank">`NotLeaderException`</a>
...
__the asynchronous system model__). NanoRaft offers another policy,
...
In NanoRaft, a follower does not vote for another Raft node if it has received
...
Therefore, users of NanoRaft should carefully take operational challenges into
...
the failed one, and its further queries return non-monotonic results. NanoRaft
...
__Note:__ Before the version 0.6, NanoRaft also had
`QueryPolicy#BOUNDED_STALENESS` which was incorrectly named for the guarantees
it offered. This policy was deleted in <a href="https://github.com/NanoRaft/NanoRaft/commit/543fc2022072381de448218da243f9254f635720" target="_blank">this commit</a>.

## What is next?

In the next post, we will explore how NanoRaft implements membership changes.
